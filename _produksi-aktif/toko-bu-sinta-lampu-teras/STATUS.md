# Status Produksi — Toko Bu Sinta — Lampu Teras

- **Status:** `approved`
- **Channel:** Toko Bu Sinta (`channel-toko-bu-sinta`)
- **Model konten:** Gambar Statis & Caption (`gambar-caption`)
- **Tahap terakhir selesai:** Tahap 5 (Generate/Acquire Assets) — G1 diterima 2026-09-17
- **Tahap berikutnya:** Tahap 6 (Assembly & Publish Prep)
- **Output resmi:**
  - `naskah-draft.md` ✅
  - `breakdown-output.md` ✅
  - `assets/unit-1-lampu-teras.png` ✅ (2.1 MB)
- **Sumber konteks yang dibaca:** `channel-brief.md`, `model-konten/gambar-caption/brief.md`, `konsistensi-visual/bu-sinta/bank-konsistensi.md`, `arsip-naskah/indeks.md`, `05_CONTENT_PRODUCTION_PIPELINE.md`, `06_PROMPT_LIBRARY.md`
- **Sumber eksternal dipakai:** Tidak ada
- **Keputusan baru:** Tema "Lampu Teras" dipilih; angle eksterior malam diterapkan; asset diterima pemilik
- **Approval yang sudah diberikan:**
  - `G1 Tahap 1+2:` disetujui 2026-09-17 — konsep & naskah diterima
  - `G2 naskah final:` disetujui 2026-09-17 — naskah 5 baris dikunci
  - `G1 Tahap 4:` disetujui 2026-09-17 — breakdown benar
  - `G2 breakdown:` disetujui 2026-09-17 — breakdown dikunci sebagai dasar generate
  - `G1 Tahap 5:` disetujui 2026-09-17 — asset diterima, sesuai breakdown
  - `G3 merge:` belum
- **Commit terakhir:** —
- **PR terkait:** —
- **Pekerjaan belum tersimpan:** Tidak ada
- **Risiko atau blocker:** —
- **Waktu pembaruan:** 2026-09-17

## Aturan

- Perbarui file ini setelah setiap tahap yang menghasilkan dependency baru.
- Jangan menyatakan tahap tersedia untuk sesi berikutnya sebelum output sudah tersimpan di branch.
- Jika status atau output tidak dapat diverifikasi setelah sesi terputus, berhenti dan minta klarifikasi.
- Field `Pekerjaan belum tersimpan`: nilai WAJIB EXACT `Tidak ada` (case-sensitive, tanpa backtick); jika ada yang belum tersimpan, isi daftar path-nya. Provenance: _meta/PROTOKOL_CHECKPOINT_RECOVERY.md di master — aturan sudah turun ke template ini; file induk tidak diperlukan.
