...
> **G2 — menunggu approval.** Gaya harus tunduk pada aturan lantai (`_sistem/03`): judul assertion, coherence (tanpa hiasan tak perlu), redundancy (naskah masuk catatan, bukan slide), segmenting (1 pesan/slide), signaling. Semua pilihan estetika di bawah ditandai dasarnya; yang `[PENILAIAN AGENT]` boleh kamu ganti.
...
- **Mode gambar: M0** di hampir semua slide; **S8** memakai **tabel perbandingan** (struktur, bukan gambar berlisensi). Tidak ada M3/M4.
- Alasan M0: Mayer (coherence) — gambar dekoratif menambah beban kognitif; topik fikih tidak butuh foto.
...
## Yang TIDAK dipakai
- Gambar AI (M3), gambar internet (M4), ikon dekoratif, animasi muncul-per-baris (tidak bisa diverifikasi tanpa renderer).
