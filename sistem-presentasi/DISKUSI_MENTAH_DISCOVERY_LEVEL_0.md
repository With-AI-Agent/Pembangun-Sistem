...
| Pengguna upload ulang dengan nama sederhana `Tesis.pdf`, tetap tidak sampai; ia mengusulkan mekanisme GitHub storage | **Upload bawaan lmarena gagal 2x berturut-turut** (folder `/home/user/uploads/` tidak pernah ada). **Mekanisme GitHub TERBUKA dan terverifikasi:** `api.github.com` HTTP 200; `GET .../contents/<path>` mengembalikan isi file base64 dan terdekode benar (uji: PANDUAN_PENGGUNA.md 5.435 bytes). Tapi `raw.githubusercontent.com` DIBLOKIR dan API-contents hanya inline sampai ±1 MB → untuk berkas besar, jalur yang pasti adalah **`git fetch`** (commit PDF ke branch). `git` ke GitHub sudah terbukti hidup sejak awal sesi (push/fetch lancar) |
| Kesiapan sisi Arab (diuji tanpa berkas asli) | **Menulis Arab ke `.pptx` TERBUKA**: python-pptx menyimpan teks Arab sebagai Unicode + `rtl="1"` + font complex-script `a:ea` + rata-kanan `algn="r"` (dicek ada di XML slide). Shaping oleh PowerPoint. **Ekstraksi teks Arab dari PDF BELUM terverifikasi** (uji sintetis tak konklusif — font penulis PDF uji tak punya glyph Arab, hasilnya titik). Jalur cadangan yang selalu jalan: render PDF→gambar (PyMuPDF) lalu baca via visi |
