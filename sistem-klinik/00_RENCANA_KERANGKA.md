...
| Lokasi | Fungsi | Tanda |
|---|---|---|
| `_bengkel/README.md` | aturan panggung inap: copy hidup di branch saja, TIDAK pernah merge main meta, pulang via download/patch, PR jadi catatan (folder berisi aturan + .gitkeep — dibuat di PR ini sebagai skeleton) | [ATURAN] |
| `_fixture/sistem-kecil-sakit/` | mini-sistem dummy TANPA mekanisme (rekaan cacat dari katalog) untuk acceptance test kit end-to-end: suntik → harus menanam + melebur + rekam; suntik dua kali → harus idempoten; rusak sengaja field STATUS → detektor harus merah | [TEMPLATE] (dibangun saat `01`–`04` selesai) |
| `_arsir-run/` | (opsi bengkel saja) salinan laporan run yang perlu disimpan meta — HANYA berkas laporan, bukan isi sistem tamu | [ATURAN] |
