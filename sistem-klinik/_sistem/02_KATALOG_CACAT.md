...
1. **Wajib Panen:** Setiap kali Sistem Klinik menyelesaikan run di repo target (rawat-jalan/bengkel) dan menemui cacat struktural/prosedural yang **belum ada** di katalog ini, temuan tersebut wajib dipanen di Tahap F.
2. **Kriteria Masuk Katalog:** Cacat harus bersifat netral-domain (bisa terjadi di sistem dengan domain berbeda), memiliki gejala yang bisa diobservasi, dan perbaikannya memiliki pola yang bisa direplikasi.
3. **Mekanisme Promosi:** Agent menyertakan cacat baru tersebut sebagai usulan (PR) ke repo meta (sistem klinik), menambahkan entri ke tabel di bawah beserta Log Keputusan perubahannya.
...
- **Pola Perbaikan (Tindakan):** Salin file/aturan eksternal yang dirujuk ke dalam folder sistem, tandai sebagai turunan berlabel versi. Ubah rujukan asli menjadi provenance historis tanpa *backtick*.
- **Risiko jika dibiarkan:** Sistem akan rusak dan tidak bisa dioperasikan saat diunduh (extracted) sebagai repo mandiri yang terpisah dari induknya.
...
- **Pola Perbaikan (Tindakan):** Sinkronisasikan field manifest dengan status sebenarnya. Tambahkan cek manifest pada alur verifikasi penutup.
- **Risiko jika dibiarkan:** Kehilangan kepercayaan terhadap manifest; agent atau sistem lain akan mengambil keputusan berdasar konteks yang salah.
...
- **Pola Perbaikan (Tindakan):** Ubah status log tersebut secara retrospektif menjadi `CLOSED` dengan menambahkan catatan penutupan di bagian akhir log (append).
- **Risiko jika dibiarkan:** Agent baru yang memulai sesi akan bingung menentukan *entry point* atau mencoba melanjutkan konteks pekerjaan yang sebenarnya sudah usai, menyebabkan tabrakan state.
...
- **Pola Perbaikan (Tindakan):** Ganti metrik volatil dengan uji keberadaan struktur (file eksis), regex pola tertentu, atau status kembalian alat (exit 0). 
- **Risiko jika dibiarkan:** Sistem akan sering mengalami *false negative* (ujian gagal padahal sistem tidak rusak) seiring pertumbuhan konten.
...
- **Pola Perbaikan (Tindakan):** Tanamkan kebijakan *fail-closed* di dokumen alur atau panduan utama, wajibkan gerbang validasi (contoh: *exit code* wajib 0 sebelum membuat PR).
- **Risiko jika dibiarkan:** Kerusakan data persisten atau korupsi *state* sistem yang jauh lebih sulit diperbaiki.
...
- **Pola Perbaikan (Tindakan):** Batasi kebijakan penyalinan branch perbaikan. File *exclude* (misal `.gitignore` atau filter `.rsync`) wajib melompati aset berat. PR perbaikan diatur hanya berisi teks pelaporan/tindakan.
- **Risiko jika dibiarkan:** Kapasitas repo bengkak secara permanen (bloat), memperlambat setiap operasi `git clone` dan `git fetch`.
