...
**Status Bagian B — DIKERJAKAN 15 September 2026** (branch `arena/01a0a2ee-pembangun-sistem`, PR tanpa auto-merge):
1. `_meta/00_CARA_KERJA_META.md` — dua langkah **WAJIB-BERTAJUK** ditanam: alur "Membangun Sistem Baru dari Nol" (setelah Langkah 1 Discovery) dan alur "Melanjutkan/Mengaudit Sistem Lama" (langkah 2). **Penyimpangan yang dicatat sadar (butuh veto pemilik bila tidak setuju):** meta TIDAK menyimpan salinan kedua `_sistem/05_TAWARAN_KAPABILITAS.md`. Alasan: (a) norma repo ini sendiri — dokumen kembar = amnesia terorganisir (Kebijakan Lebur Aturan 1, dan butir "rujukan ber-backtick ke _meta/ wajib salinan berlabel" berlaku untuk folder sistem, bukan sebaliknya); (b) folder klinik berada di repo yang sama dengan meta, jadi "dua-duanya disinkron lewat AT-KL" tidak diperlukan — cukup SATU sumber + rujukan path penuh, dan justru mustahil basi. Bila pemilik tetap menghendaki salinan di `_meta/`, ini dibalik — keputusan pemilik.
2. `_meta/01_DISCOVERY_LEVEL_0.md` — poin ke-6 **KAPABILITAS EKSTERNAL** ditambahkan (satu pertanyaan; jawabannya masuk rencana kerangka).
3. `_meta/FAILURE_INJECTION_TESTS.md` — deskripsi baris R4 dilengkapi dengan pengecualian `Tahap: kerangka` (catatan reviewer PR #43 yang ikut tercatat di sini).
4. Versi meta v1.13.2 → v1.14.0 + baris Log Evolusi; regresi penuh dijalankan.
