...
- **Dependency eksternal:** akses sesi agent (mayoritas lmarena; kadang Claude Code/Antigravity — aturan netral-platform), akses internet untuk riset Tahap 1/3, `git` + `gh` untuk branch/PR.
  - **Kapabilitas terpasang (vendor-local, susulan K-10 2026-09-15 — audit 10 zip Input-Pengguna):**
    - `skills/frontend-designer` v3.0.1 Apache-2.0 — design-system/tokens/CSS/a11y (dari `frontend-designer-skill-main.zip` 80K→24K) — dipakai TECH_SPEC & Coding UI.
    - `skills/excalidraw-diagram` — diagram argue-visually + render Playwright (dari `excalidraw-diagram-skill-main.zip` 19K→57K) — dipakai DISCOVERY/TECH_SPEC/ROADMAP.
    - `skills/ui-ux-pro-max` core (1.7M selective dari 8.1M) + `skills/design` + `skills/design-system` + `skills/ui-styling` — reasoning tokens/styles + shadcn/Tailwind (prune 1.9M font data per C-06).
    - Lihat `skills/README.md` & `_sistem/02_TAWARAN_KAPABILITAS_PLUS_AUDIT.md` untuk audit lengkap 10 zip, scan keamanan (`curl|bash` sandbox, `rm -rf`/`base64` legit), dan daftar kandidat tertunda (agent-skills-hub 35M, agent-skills 6.6M, ios-agent-skill 2M, alibaba-java 32K, dsb — siap selective susulan).
  - **Kandidat riset eksternal (web_search 5 sumber, belum terpasang — butuh approval susulan):** Context7/llms.txt (fresh docs) + Superpowers/Planning-with-Files + Playwright verify — lihat tabel 7 kolom di `_sistem/02_TAWARAN_KAPABILITAS_PLUS_AUDIT.md` §B.
- **Data yang wajib ada:** `/docs` (Fondasi), `PROJECT_STATE.md` (penunjuk sesi), `DECISIONS_LOG.md` (memori Area Berisiko), `ROADMAP.md` (task).
- **Risiko utama:** (1) Agent salah tebak Area Berisiko Tinggi karena tidak baca DECISIONS_LOG (mitigasi: W-05 wajib dibaca sebelum sentuh area); (2) Sesi crash tanpa LOG_SESI (mitigasi: W-02 berkelanjutan + PROJECT_STATE deterministik); (3) Scope creep di PRD (mitigasi: MoSCoW Tahap 2 + Non-Goals eksplisit); (4) Bengkak aset akibat skill besar (mitigasi: selective install + prune, sudah hemat 92% 52M→2.1M, validator cek ukuran).
