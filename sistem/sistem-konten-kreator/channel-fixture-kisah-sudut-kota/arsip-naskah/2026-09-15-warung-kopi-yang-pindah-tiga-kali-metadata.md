...
- **Catatan produksi:**
  - Produksi pertama model `Kartu Teks 8–10` — model ini dibuat di sesi yang sama dan masih berstatus `Approved` (belum `Merged`) saat produksi dijalankan. Status itu dipertahankan apa adanya, tidak dinaikkan jadi `Operational` sebelum merge. Sesuai instruksi pemilik ("setelah model terkunci, produksi 1 konten") dan preseden **PR #58** (Kata Data): brief `model-konten/data-60-detik/brief.md` dan unit produksinya dikirim dalam satu PR yang sama dan brief-nya tetap berstatus `Approved`. **Koreksi (temuan T-4b review PR #60):** PR #54 **bukan** preseden untuk hal ini — PR #54 hanya mencentang kotak status `narasi-riset-60-detik`; isi brief itu sudah masuk `main` lebih dulu lewat PR #52.
