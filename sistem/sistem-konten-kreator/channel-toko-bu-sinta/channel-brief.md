# Channel Brief — Toko Bu Sinta

### Dokumen "hidup" milik SATU channel, mengikuti struktur `_sistem/03_TEMPLATE_CHANNEL_BRIEF.md`.

---

## Status: `Draft` → `Reviewed` → `Approved` → `Merged` → `Operational`

- [x] **Draft** — digali via Channel Discovery 2026-09-17 (slot log 20)
- [x] **Reviewed** — dibaca lengkap oleh pemilik via viewer, 2026-09-17
- [x] **Approved** — isi dikunci lewat gerbang **G2** 2026-09-17 oleh pemilik
- [x] **Merged** — PR #76 masuk ke `main` pada 2026-09-17 oleh pemilik
- [x] **Operational** — dinaikkan 2026-09-17 (sesi 25); semua dependency checklist terpenuhi

**Checklist Kelengkapan — syarat naik ke `Operational`:**

- [x] Semua bagian wajib di bawah terisi (tidak ada placeholder tersisa)
- [x] Semua pertanyaan di checklist konsistensi (bagian 4) sudah dijawab
- [x] Setiap elemen visual yang ditandai wajib sudah berstatus `Reference-Ready` (`konsistensi-visual/bu-sinta/` memiliki acuan utama lolos audit)
- [x] `arsip-naskah/indeks.md` dan `arsip-naskah/indeks-karakter.md` sudah dibuat
- [x] Sudah `Merged` ke `main`

**Versi:** `1.1` — **Terakhir diperbarui:** `2026-09-19` — **Diwarisi dari:** `_sistem/01_BRAND_CORE.md`

> **Catatan dependency:** `_sistem/01_BRAND_CORE.md` di repo ini masih template umum belum terisi spesifik. Nilai-nilai channel ini dikunci mandiri di bagian 2, 3, dan 5 di bawah; bila Brand Core kelak diisi, brief ini dapat diselaraskan via G2 terpisah.

---

## 1. Identitas Channel

- **Nama channel:** Toko Bu Sinta
- **Slug repo:** `channel-toko-bu-sinta`
- **Platform utama:** Instagram (post feed)
- **Format teknis:** [ ] Short-form  /  [ ] Long-form  /  [x] Gambar & carousel (1 gambar ilustrasi flat + caption 3–6 baris)  /  [ ] Campuran

## 2. Niche & Positioning

- **Topik inti (sudut pandang spesifik, bukan cuma judul besar):** Cerita-cerita kecil yang hangat dari keseharian sebuah toko kelontong kecil di lingkungan warga. Sudut pandang naratif selalu berpusat pada benda-benda sehari-hari (stoples kaca, cangkir seng, bel pintu, timbangan beras) atau interaksi kecil yang tulus di dalam toko.
- **Masalah/kebutuhan/hiburan yang dipenuhi buat penonton:** Menghadirkan rasa tenang, nostalgia kehangatan toko kelontong tetangga, dan jeda sejenak (healing) yang bersahaja di tengah hiruk-pikuk media sosial; tanpa dramatisasi berlebih, tanpa bising.
- **Target penonton (kondisi/minat spesifik, bukan cuma demografi):** Orang Indonesia usia 25–45 tahun yang menyukai cerita bergenre *slice-of-life*, rindu suasana toko kelontong masa kecil atau kenyamanan bertetangga yang tulus.
- **Apa yang bikin channel ini beda dari yang sudah ada:** 
  1. Sudut pandang mikro: cerita lahir dari observasi benda kecil sehari-hari dan gerak-gerik bersahaja, bukan drama besar.
  2. Bu Sinta sebagai satu-satunya jangkar visual dan emosional yang konsisten hadir di tiap konten.
  3. Format ringkas dan menenangkan: 1 gambar flat berpalet lembut dengan caption 3–6 baris yang bernas tanpa desakan engagement buatan.
  4. Narasi pihak ketiga yang bersahaja seperti teman lama yang duduk di sudut toko.
- **Referensi/kompetitor yang relevan (untuk dipelajari, bukan ditiru):** Akun-akun komik pendek dan ilustrasi *slice-of-life* bertema keseharian kedai atau toko tradisional (baik lokal maupun referensi Jepang/Korea yang menonjolkan kehangatan mikro). Dipelajari keheningan ritme dan kemampuannya membangkitkan empati tanpa kata-kata berlebihan.

## 3. Persona & Voice Channel (WAJIB DIISI — berlaku untuk SEMUA channel, termasuk faceless)

- **Siapa "suara" di balik channel ini?** [x] Narator/voice over tanpa wujud visual tetap — narator orang ketiga yang hangat seperti teman lama yang mengamati suasana toko dengan penuh perhatian. Bu Sinta adalah tokoh utama yang hadir di gambar dan berbicara lewat ucapan-ucapan pendek yang dikutip narator.
- **Gaya bahasa:** Santai, bersahaja, hemat kata, menggunakan kalimat-kalimat pendek sederhana. Ritme lambat dan mengalir tenang. Tidak menggunakan bahasa gaul (slang anak muda), tidak puitis berlebihan, tidak ada desakan sensasional.
- **Tone emosional dominan:** Hangat, tenang, humor pelan (subtle), tidak mendramatisir, tidak menggurui.
- **Kosakata/frasa khas yang sering dipakai:** "di toko kami", "dulu", "yang penting", "pelan-pelan aja", "uang bisa dicari, teman susah".
- **Karakteristik suara/voice:** Tidak berlaku (channel ini murni visual gambar + teks caption, tanpa audio / TTS).
- **Hal yang HARUS ADA di setiap konten channel ini:**
  - Bu Sinta (Karakter Utama) wajib tampil di gambar.
  - Format 1 gambar ilustrasi flat + caption 3–6 baris.
  - Struktur ritme caption: baris 1–2 pengamatan benda/situasi toko, baris 3–4 interaksi kecil atau ucapan singkat Bu Sinta, baris 5–6 refleksi penutup pendek yang hangat.
  - **Aturan penutup wajib (mengikat semua konten):** Baris penutup caption HARUS berupa observasi terhadap benda yang menjadi fokus konten (benda yang sama yang muncul di gambar dan dibuka di baris awal). Dilarang menutup dengan generalisasi, nasihat, atau refleksi yang melompat ke pembaca/kehidupan umum. Penutup mendarat kembali ke benda, tidak melantur ke pesan universal.
  - Suasana toko kelontong yang bersahabat dan tenang.
- **Hal yang TIDAK BOLEH ADA / dihindari di channel ini:**
  - Nama toko komersial atau brand nyata (hindari pelanggaran hak cipta dan kesan endorsement).
  - Judul/caption sensasional, umpan klik (clickbait), atau gaya tulisan memancing amarah.
  - Nasihat hidup menggurui atau khotbah moral eksplisit.
  - **Penutup caption yang menggeneralisasi ke pembaca atau melompat ke nasihat/renungan umum** (mis. pola "Di toko kami, …" sebagai penutup, "Kadang tanda paling sederhana …", "begitu juga hidup …", "pelan-pelan aja …" tanpa objek benda yang dirujuk, dan sejenisnya yang meninggalkan benda fokus). Penutup wajib tetap berada di benda — observasi konkret, bukan wejangan.
  - Format video atau suara latar (voice over).
  - Humor kasar, berisik, atau slapstick.
- **Contoh kalimat pembuka/penutup khas:**
  - *Pembuka:* "Di toko kami, stoples kopi ini jarang sekali bersuara keras." / "Pagi di toko kami selalu dimulai dari bunyi bel pintu yang sedikit serak." / "Di sudut dekat timbangan, selalu ada cerita yang tertinggal."
  - *Penutup (pola yang BENAR — mendarat ke benda fokus):* "Stoples itu tinggal separuh, tapi aromanya masih memenuhi sudut meja kasir." / "Bel pintu tadi sudah berbunyi delapan kali — besok pagi ia akan berbunyi lagi." / "Timbangan bebek itu diam di tempatnya, jarumnya kembali ke nol seperti biasa."
  - *Penutup (pola yang TIDAK BOLEH — generalisasi/nasihat yang meninggalkan benda):* ~~"Yang penting, hari ini toko tutup tepat waktu."~~ / ~~"Pelan-pelan aja, besok pagi toko buka lagi."~~ / ~~"Kadang tanda paling sederhana adalah yang paling berarti."~~

## 4. Konsistensi Visual

**Karakter — Tipe A (Karakter Utama Channel)**

> `konsistensi-visual/bu-sinta/` — Bu Sinta (52 tahun, pemilik toko). SATU-SATUNYA karakter permanen channel ini. **WAJIB tampil di SETIAP gambar konten**. Detail fisik, kepribadian, gaya bicara, Prompt Master, dan berkas acuan visual dikelola via `04_CHARACTER_BUILDER_KIT.md` di folder tersebut. Rujukan gaya bicara terkait erat dengan bagian 3 di atas.

**Karakter — Tipe B (Karakter Per-Konten)**

> Pelanggan, tetangga, anak-anak, atau pedagang keliling yang singgah di toko. Mereka hadir situasional sebagai karakter per-konten (Tipe B): wajib konsisten di dalam konten terkait, didaftarkan pada `arsip-naskah/indeks-karakter.md`, dan dicek sebelum produksi berikutnya untuk menimbang potensi naik kelas ke Tipe A.

**Latar/Lingkungan**

> [x] Tidak berlaku untuk channel ini (Latar interior/eksterior toko kelontong seperti rak dagangan, meja kayu, dan jajaran stoples hadir fleksibel dan alami di latar belakang. Bu Sinta adalah jangkar visual utama; tidak memerlukan reference pack latar terpisah).

**Palet Warna & Gaya Render**

> [x] Tidak berlaku untuk channel ini (Gaya render ilustrasi flat dengan warna soft-teredam dan pencahayaan pagi/sore sudah dikunci langsung di Bagian 5 di bawah serta tercermin di Prompt Master Bu Sinta).

**Props/Objek Berulang**

> [x] Tidak berlaku untuk channel ini (Detail benda khas seperti toples kaca kopi tua, timbangan beras bebek, dan cangkir seng blirik hijau melekat langsung ke deskripsi karakter Bu Sinta dan adegan toko tanpa folder reference pack terpisah).

## 5. Gaya Visual

- **Referensi visual/mood:** Ilustrasi *flat* 2D (flat illustration), garis bersih, tekstur lembut tanpa efek 3D, pencahayaan alami yang hangat khas sinar matahari pagi (golden morning light) atau sore hari menjelang magrib (soft warm afternoon sunlight).
- **Palet warna dominan:** Warna-warna bersahaja teredam (soft muted earth tones) — kuning mustard, terakota hangat, krem kertas tua, cokelat kayu, hijau sage lembut, dengan aksen merah lembut pada selendang Bu Sinta.
- **Pendekatan generate visual untuk channel ini:** WAJIB menyertakan berkas gambar referensi Bu Sinta (`konsistensi-visual/bu-sinta/referensi/acuan-utama.png`) yang digabungkan dengan Prompt Master Bu Sinta + deskripsi situasi/adegan spesifik konten. Rasio gambar persegi 1:1 (Instagram post feed).
- **Hal yang harus dihindari secara visual:** Gaya 3D fotorealistik atau hiper-realis; gaya anime shonen garis tajam tebal; warna-warna neon jenuh tinggi; teks atau tipografi di dalam gambar (semua teks berada di caption); logo atau tulisan merek produk nyata pada barang dagangan.

## 6. Area Berisiko Tinggi — hal yang gampang "ditebak ulang" beda oleh AI

- **Atribut wajib Bu Sinta terlupakan:** Selendang merah dan kacamata berbingkai cokelat tipis wajib selalu ditegaskan dalam prompt agar tidak hilang.
- **Pakaian Bu Sinta melenceng:** Kardigan kuning mustard di atas kemeja putih sering kali tertukar warna jika tidak disebut eksplisit.
- **Gaya caption menjadi ceramah/quotes motivasi:** AI sering kali tergoda membuat kalimat penutup bernada sok bijak; wajib ditegaskan agar tetap santai, membumi, dan tidak menggurui.
- **Ucapan Bu Sinta melenceng menjadi bahasa gaul/slang modern:** Wajib ditegaskan bahwa Bu Sinta adalah wanita berumur 52 tahun yang bicaranya tenang, pendek, dan bersahaja ("pelan-pelan aja", "uang bisa dicari, teman susah").
- **Kemunculan brand produk nyata di rak toko:** Gambar rak barang rentan menampilkan kemasan merek terkenal; prompt harus selalu mengarahkan ke kemasan polos atau toples kaca tanpa label merk komersial nyata.

## 7. Bank Ide Awal

1. Pelanggan tua yang selalu singgah membawa cucunya membeli permen jahe.
2. Stoples kopi tua di meja kasir yang tidak pernah dibiarkan kosong oleh Bu Sinta.
3. Kipas angin dinding tua yang bunyinya berderit pelan menemani siang yang sepi.
4. Uang kembalian yang diganti permen manis saat recehan koin sedang habis.
5. Catatan kasbon di buku tulis bergaris yang sudutnya terlipat dan lusuh.
6. Hujan lebat mendadak yang membuat beberapa orang berteduh di bawah teras toko sambil saling sapa.

## 8. Model Konten dalam Channel Ini

| Nama Model Konten | Folder | Status |
|---|---|---|
| Gambar Statis & Caption | `model-konten/gambar-caption/` | `Operational` — dinaikkan 2026-09-17 (sesi 25) |

## 9. Arsip Naskah

Folder `arsip-naskah/` disediakan untuk menyimpan naskah final setiap post yang publish, dilengkapi dengan `indeks.md` (kronologi konten) dan `indeks-karakter.md` (pelacakan karakter Tipe B per-konten). Setiap konten yang selesai mengarsipkan naskah + metadata reproduksibilitas sesuai `05_CONTENT_PRODUCTION_PIPELINE.md`.

## 10. Log Keputusan Channel

| Tanggal | Keputusan | Alasan |
|---|---|---|
| 2026-09-17 | Inisiasi Channel Discovery Toko Bu Sinta; penetapan platform Instagram feed (1 gambar + caption 3–6 baris) | Brief awal pemilik: channel bertema cerita kecil hangat toko kelontong, tanpa video/VO. |
| 2026-09-17 | G1 Lulus & G2 Approved — Channel Brief Toko Bu Sinta v1 dikunci resmi | Pemilik menyetujui penuh draft tanpa revisi. |
| 2026-09-17 | Bu Sinta (Karakter Tipe A) Reference-Ready; acuan-utama.png terverifikasi di repo | Tahap 2 Character Builder Kit tuntas; seluruh atribut wajib terpasang sempurna. |
| 2026-09-17 | Bu Sinta dikunci sebagai satu-satunya Karakter Utama (Tipe A); wajib tampil di tiap konten | Karakter sentral yang menjadi pengikat emosional dan jangkar visual channel. |
| 2026-09-17 | Latar dan props ditandai tidak berlaku untuk folder reference pack terpisah; menyatu ke prompt adegan dan Prompt Master | Pilihan pemilik di validasi poin 1 & 3: Bu Sinta adalah jangkar visual utama; latar toko dan detail meja hadir fleksibel tanpa reference pack terpisah. |
| 2026-09-17 | Struktur caption dibakukan: baris 1-2 pengamatan benda/situasi, baris 3-4 interaksi/ucapan Bu Sinta, baris 5-6 refleksi penutup pendek | Kesepakatan pada validasi poin 2 untuk menjaga ritme konsistensi narasi tanpa hard CTA. |
| 2026-09-17 | Batasan keras: tanpa nama toko/brand nyata, tanpa clickbait, tanpa bahasa gaul | Menjaga integritas slice-of-life bersahaja dan kepatuhan hak cipta. |
| 2026-09-17 | Status naik ke `Merged` — PR #76 masuk `main` oleh pemilik | Channel Brief dan seluruh dependency (Bu Sinta Reference-Ready, indeks arsip) sudah ada di `main`. |
| 2026-09-17 | Status naik ke `Operational` — sesi 25 | Semua checklist kelengkapan terpenuhi: bagian wajib terisi, checklist konsistensi terjawab, Bu Sinta Reference-Ready, indeks arsip ada, sudah merged. Channel siap produksi. |
| 2026-09-19 | Aturan baru (mengikat semua konten): baris penutup caption WAJIB berupa observasi terhadap benda fokus konten; dilarang generalisasi/nasihat ke pembaca (pola "Di toko kami, …" atau "Kadang tanda paling sederhana …" sebagai penutup tidak boleh lagi dipakai). Contoh penutup lama yang melanggar dicoret dan diganti contoh yang mendarat ke benda. | Pemilik ingin penutup caption tetap berada di benda, tidak melompat ke nasihat/renungan umum — menjaga disiplin sudut pandang mikro yang menjadi pembeda channel. |
