...
| 9 Sep 2026 | **0.3.10** | Persiapan clean-run G-1 (keputusan pemilik 9 Sep 2026 — pengecualian TERCATAT F-1; basis main `3eb053e` = merge PR #30, KK `0.3.9`): (1) unit produksi baru `_produksi-aktif/fixture-narasi-sejarah-pintu-kos-yang-tidak-pernah-dikunci/` (Detail state diredaksi per pola 6d; bukti: ACCEPTANCE_TEST_LOG.md / riwayat commit); (2) arsip fixture `2026-09-09-penjual-bunga-di-pasar-subuh` (Detail state diredaksi per pola 6d; bukti: ACCEPTANCE_TEST_LOG.md / riwayat commit); (3) channel fixture baru `channel-fixture-kisah-sudut-kota/` (brief + model + arsip kosong) — [label uji diredaksi per pola 6d]; (4) Channel Brief Narasi Sejarah v2 (contoh kalimat pembuka) — [pemetaan sisi/run diredaksi per pola 6d]; (5) dokumen baru `UJI_G1_CLEAN_RUN_2026-09-09.md` (orkestrasi: prompt subjek netral + pencatat + state + protokol); (6) entri append-only "Persiapan G-1" di `ACCEPTANCE_TEST_LOG.md`; (7) higiene 3 header status uji yang basi (F7 DITUTUP); (8) `validate_system.py`: pengecualian declared gap warisan di konsistensi arsip — `indeks-karakter.md` yang TIDAK ADA hanya diakui kalau channel-brief.md menyatakan marker eksak "gap warisan: indeks-karakter.md belum pernah dibuat" (tanpa pernyataan = tetap FAIL; `indeks.md` tidak pernah dikecualikan; baris laporan "gap warisan" ditambahkan). **Dokumen aturan `00`/`05`/`06` TIDAK diubah** — perubahan = gerbang folder (validator) + state uji + orkestrasi; [status suite diredaksi per pola 6d; detail: ACCEPTANCE_TEST_LOG.md section 'Clean-run G-1 — Run 9–18' / riwayat commit] | Kontradiksi struktural diselesaikan: [premis/pengecualian uji diredaksi per pola 6d; bukti: ACCEPTANCE_TEST_LOG.md / riwayat commit]. Regresi penuh (validator repo, FI, backup, build template, gerbang mandiri, diff aturan = 0 baris) dicatat di body PR | Menunggu review independen (perubahan validator/gerbang + state uji) — PR tanpa auto-merge; penggabungan = keputusan pemilik. Rollback: revert commit persiapan (state fixture + pengecualian validator + orkestrasi); tanpa migrasi data |
