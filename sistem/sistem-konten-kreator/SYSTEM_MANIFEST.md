...
- **Acceptance test:** **Clean-run G-1 (Run 9–18, `0.3.10`, dicatat 15 Sep 2026 oleh pencatat sesi terpisah): 5 kode LULUS pada `0.3.10`** (AT-KK-01 / Run 13, AT-KK-03 / Run 11, AT-KK-03b / Run 12, AT-KK-04 / Run 15, AT-KK-07 / Run 17) + **5 kode GAGAL-metode** (AT-KK-05 / Run 9, AT-KK-05b / Run 10, AT-KK-02 / Run 14, AT-KK-06 / Run 16, AT-KK-08 / Run 18) → **gate `Operational` + backlog G-1 TETAP TERBUKA**; re-run 5 kode dijadwalkan lewat **sesi terpisah**. Sebelumnya: **F7 DITUTUP 6 Sep 2026** — AT-KK-05 / Run 7 **LULUS** + AT-KK-05b / Run 8 **LULUS**, keduanya `0.3.4` (LULUS terakhir dua kode itu tetap `0.3.4`, tidak ditimpa). Riwayat: Run 4 **GAGAL** (`0.3.2`); Run 5 **GAGAL-metode** (`0.3.3`, koreksi pasca-review); Run 6 lama **void**. Status/bukti: `ACCEPTANCE_TEST_LOG.md` (bagian batch G-1) + tabel Rekaman Hasil `ACCEPTANCE_TESTS.md`.
...
- [ ] Acceptance test sistem ini LULUS — **belum** (syarat: seluruh baris LULUS pada versi sistem yang sama). Clean-run G-1 pada `0.3.10` (15 Sep 2026): **LULUS** AT-KK-01/03/03b/04/07 (Run 13/11/12/15/17); **GAGAL-metode** AT-KK-05/05b/02/06/08 (Run 9/10/14/16/18) — perilaku tercatat sebagai observasi, metode tidak bersih, **perlu re-run**. AT-KK-05 / Run 7 dan AT-KK-05b / Run 8 tetap **LULUS terakhir** pada `0.3.4`; Run 5 `0.3.3` GAGAL-metode; Run 6 lama void. Status/bukti: `ACCEPTANCE_TEST_LOG.md` (bagian batch G-1).
...
| L-05 acceptance test dapat diulang | P2 | **Sebagian**. Clean-run G-1 pada `0.3.10` (Run 9–18, dicatat 15 Sep 2026 oleh pencatat sesi terpisah): 5 kode **LULUS** (AT-KK-01/03/03b/04/07), 5 kode **GAGAL-metode** (AT-KK-05/05b/02/06/08) — seluruh 10 skenario kini **pernah dijalankan**, tapi belum pada satu versi yang sama untuk semua, jadi gate `Operational` **tetap terbuka**. Sebelumnya: AT-KK-05 / Run 7 **LULUS** + AT-KK-05b / Run 8 **LULUS**, keduanya `0.3.4` — **F7 DITUTUP 6 Sep 2026**. Riwayat: Run 4 **GAGAL** (`0.3.2`); Run 5 **GAGAL-metode** (`0.3.3`); Run 6 lama **void**. Status/bukti: `ACCEPTANCE_TEST_LOG.md` (bagian batch G-1) |
...
| 9 Sep 2026 | **0.3.10** | Persiapan clean-run G-1 (keputusan pemilik 9 Sep 2026 — pengecualian TERCATAT F-1; basis main `3eb053e` = merge PR #30, KK `0.3.9`): (1) unit produksi baru `_produksi-aktif/fixture-narasi-sejarah-pintu-kos-yang-tidak-pernah-dikunci/` (Tahap 3 selesai, naskah r1 135 kata, G1 Tahap 1/2/3 disetujui, G2 naskah final belum — state retest AT-KK-05/05b); (2) arsip fixture `2026-09-09-penjual-bunga-di-pasar-subuh` (naskah final + karakter Tipe B "Nenek Penjual Bunga" tercatat di `indeks-karakter.md`) — state AT-KK-03; (3) channel fixture baru `channel-fixture-kisah-sudut-kota/` (brief + model + arsip kosong) — state AT-KK-03b/08; (4) Channel Brief Narasi Sejarah v2 (contoh kalimat pembuka) — sisi A AT-KK-06 (sisi B = branch `uji-06-branch-b` di remote); (5) dokumen baru `UJI_G1_CLEAN_RUN_2026-09-09.md` (orkestrasi: prompt subjek netral + pencatat + state + protokol); (6) entri append-only "Persiapan G-1" di `ACCEPTANCE_TEST_LOG.md`; (7) higiene 3 header status uji yang basi (F7 DITUTUP); (8) `validate_system.py`: pengecualian declared gap warisan di konsistensi arsip — `indeks-karakter.md` yang TIDAK ADA hanya diakui kalau channel-brief.md menyatakan marker eksak "gap warisan: indeks-karakter.md belum pernah dibuat" (tanpa pernyataan = tetap FAIL; `indeks.md` tidak pernah dikecualikan; baris laporan "gap warisan" ditambahkan). **Dokumen aturan `00`/`05`/`06` TIDAK diubah** — perubahan = gerbang folder (validator) + state uji + orkestrasi; suite clean-run G-1 (10 run) di-rebaseline ke `0.3.10` dan memvalidasi versi ini | Kontradiksi struktural diselesaikan: premise AT-KK-03b (channel yang TIDAK PERNAH membuat `arsip-naskah/indeks-karakter.md` — state yang diantisipasi eksplisit oleh aturan 06 bagian A2: "channel baru, atau arsip dibuat sebelum aturan ini") membuat gerbang folder FAIL kalau channel itu ada di `main`; pengecualian declared menjaga gigi gerbang (ketiadaan tanpa pernyataan tetap gagal). Regresi penuh (validator repo, FI, backup, build template, gerbang mandiri, diff aturan = 0 baris) dicatat di body PR | Menunggu review independen (perubahan validator/gerbang + state uji) — PR tanpa auto-merge; penggabungan = keputusan pemilik. Rollback: revert commit persiapan (state fixture + pengecualian validator + orkestrasi); tanpa migrasi data |
| 15 Sep 2026 | **0.3.10** | Pencatatan clean-run **G-1**: Run 9–18 ditulis append-only ke `ACCEPTANCE_TEST_LOG.md` (10 bagian run + temuan lintas-run F1–F8 + rencana re-run 5 kode) oleh **pencatat sesi terpisah**; tabel Rekaman Hasil `ACCEPTANCE_TESTS.md`, baris Sistem Konten Kreator di _meta/INDEKS_SISTEM.md (provenance, area master), dan berkas ini disinkronkan **status + versi + pointer saja**. **Tidak ada dokumen aturan (`00`/`05`/`06`, `ACCEPTANCE_TESTS.md`) yang diubah** dan versi sistem tidak naik. | 5 kode **LULUS** pada `0.3.10` (AT-KK-01/03/03b/04/07); 5 kode **GAGAL-metode** (AT-KK-05/05b/02/06/08) — perilaku tercatat sebagai observasi, metode tidak bersih; **gate `Operational` + backlog G-1 TETAP TERBUKA**; perbaikan orkestrasi + re-run = sesi terpisah (bukan pencatat). Bukti: `ACCEPTANCE_TEST_LOG.md` bagian batch G-1 |
| 9 Sep 2026 | **0.3.9** | Perubahan mekanisme log sesi (permintaan pemilik 9 Sep 2026 — root repo terasa berantakan): aturan lokasi di aturan 00 (00_CARA_PAKAI_SISTEM.md) — sesi lintas sistem kini menuliskan log ke folder `_log-sesi/` di root repo, bukan di root langsung; prompt universal pegangan + prompt entri (W-01) di-update mengikuti. **Aturan 00 berubah (aturan lokasi saja, bukan aturan produksi) → regresi AT-KK berlaku**: re-run dijalankan berdasar backlog G-1 (8 test belum diuji + AT-KK-05/05b pada versi yang sama) | Bukti: PR D (sesi ini) — gerbang mandiri + regresi penuh PASS. **Menunggu review independen L1** — PR tanpa merge |
