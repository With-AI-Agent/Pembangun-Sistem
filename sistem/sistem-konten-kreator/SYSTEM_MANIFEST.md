...
| 15 Sep 2026 | **0.3.10** | Pencatatan clean-run **G-1**: Run 9–18 ditulis append-only ke `ACCEPTANCE_TEST_LOG.md` (10 bagian run + temuan lintas-run F1–F8 + rencana re-run 5 kode) oleh **pencatat sesi terpisah**; tabel Rekaman Hasil `ACCEPTANCE_TESTS.md`, baris KK `_meta/INDEKS_SISTEM.md`, dan berkas ini disinkronkan **status + versi + pointer saja**. **Tidak ada dokumen aturan (`00`/`05`/`06`, `ACCEPTANCE_TESTS.md`) yang diubah** dan versi sistem tidak naik. | 5 kode **LULUS** pada `0.3.10` (AT-KK-01/03/03b/04/07); 5 kode **GAGAL-metode** (AT-KK-05/05b/02/06/08) — perilaku tercatat sebagai observasi, metode tidak bersih; **gate `Operational` + backlog G-1 TETAP TERBUKA**; perbaikan orkestrasi + re-run = sesi terpisah (bukan pencatat). Bukti: `ACCEPTANCE_TEST_LOG.md` bagian batch G-1 |
