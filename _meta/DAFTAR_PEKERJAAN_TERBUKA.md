...
| T-28 | **Kelayakan upscaling raster + vectorization di lingkungan ini BELUM DIUJI** | Muncul dari keputusan G3 (jalur "LOLOS BERSYARAT" bergantung pada kemampuan menaikkan resolusi foto). **Yang sudah diketahui menghambat:** `pip install` tidak bertahan antar sesi · tidak ada ffmpeg/ImageMagick-PDF/chromium · **semua API layanan eksternal terblokir** (hanya npm, PyPI, github.com) sehingga jalur cloud seperti LetsEnhance **gugur sejak awal** · bobot model Real-ESRGAN harus bisa diunduh dari sumber yang terjangkau. **Jangan klaim jalur ini siap sebelum diuji** — kalau tidak bisa dijalankan, G3 jalur "LOLOS BERSYARAT" harus dinyatakan tidak tersedia dan hanya jalur LOLOS/DITOLAK yang tersisa | **P1** | TERBUKA | — |
