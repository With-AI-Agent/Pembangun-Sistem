...
**Jumlah:** 64 skenario di master (15 sintetis + 10 unit nyata + 14 regresi review PR-11 + 10 regresi check_selfcontained + 15 regresi review_prompt), 16 di ekstrak template (15 sintetis + 1 unit nyata benih; regresi repo-copy tidak dijalankan bersarang). Angka ini disalin dari baris yang dicetak `tools/test_failure_injection.py`, bukan dihitung tangan. Penambahan terakhir: RP5 jadi 5 pemeriksaan (T-2, temuan review PR #55) — **diperkuat 15 Sep 2026 setelah temuan T-4 review PR #56**: versi pertama hanya memeriksa keberadaan teks `--paginate` di sumber (tautologi); kini yang diuji adalah argv yang benar-benar dipakai `fetch_pr_files` (RP5b) dan penjaga konsistensi `resolve_pr_files` (RP5a), dua-duanya diuji-mutasi dengan penegasan bahwa mutasi yang tidak mengubah apa pun membuat uji gagal.
