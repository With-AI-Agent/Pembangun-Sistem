...
| Sistem Klinik | `sistem-klinik/` | Sedang dibangun | 14 September 2026 | Rencana kerangka merged via PR #43; isi 01–06 + kit terakit merged via PR #47/#48 (13 Sep). Langkah 7 selesai (14 Sep): `START_DI_SINI.md` + acceptance test pertama di fixture (AT-KL-01/02 lulus; bukti `ACCEPTANCE_TEST_LOG.md`); temuan AT-KL-02 diperbaiki (aturan anti-kit-usang, kit v0.1.2). Audit matkangan pra-run hijau (validator meta + gerbang mandiri + alat kit). Menyusul: ratifikasi pemilik + (opsional) flip `siap-pakai` → PR meta Bagian B → run pertama. TIDAK mengklaim mandiri sampai tahap siap-pakai. |
