...
- **Meta-sistem (`_meta/`)** — ini kerangka yang menaungi semua sistem, bukan salah satu isinya. Statusnya dilacak di `_meta/SYSTEM_MANIFEST.md` (saat ini `Released — v1.13.0`, terakhir disentuh 9 September 2026: pegangan pengguna wajib + log sesi berkelanjutan (level repo/meta kini di folder `_log-sesi/`) + kontrak warisan W-01…W-09 + tools regresi berbasis INDEKS + protokol review independen + folder sistem sebagai deliverable + cakupan gerbang mandiri diselaraskan dengan definisi dokumen aktif + dua sistem domain lama dituntaskan menjadi folder mandiri + penutupan administratif PR #27 dan pembersihan angka korpus Handoff + hasil audit Mendalam 9 Sep: 4 perbaikan dokumen + housekeeping).
