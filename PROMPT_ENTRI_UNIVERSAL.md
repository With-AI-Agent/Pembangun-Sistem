...
**Audit ISI (bukan review PR) — mekanisme yang berbeda, jangan tertukar:** kalau yang diminta adalah memeriksa *kesehatan isi* sebuah jalur (`_meta`, `tools`, sebuah sistem, satu berkas) dan **bukan** memutuskan merge, jalankan `python3 tools/audit_prompt.py --objek <path>` lalu tempel seluruh keluarannya ke sesi auditor yang terpisah. Hasilnya diserahkan auditor sebagai **berkas ter-commit** di `_meta/_internal/audit/` (Kanal A = UTAMA) dengan baris pertama `# AUDIT <objek> @<sha7>`; GitHub Issue berlabel `audit-independen` hanyalah Kanal B, alternatif yang terukur terblokir HTTP 403, dan diambil sendiri oleh sesi yang diaudit dengan `python3 tools/ambil_verdict.py --terbaru` — **pemilik cukup bilang "audit sudah selesai", tidak perlu menyalin apa pun**. **Prompt audit tidak boleh dikarang atau disunting oleh sesi yang objeknya diaudit.** Aturannya: `_meta/PROTOKOL_AUDIT_ISI.md`. Ingat bedanya: review PR menjawab *"bolehkah di-merge"*; audit isi menjawab *"sehatkah isi ini"* dan **tidak** memutuskan merge apa pun.
