...
## Identitas
- **Versi:** `0.5.1` · **Status:** Built & terverifikasi; **teraudit 1x independen** (5 Sep, 11 temuan AP-01…AP-11 semua diperbaiki di v0.3.0); **folder mandiri** (9 Sep, lolos gerbang AT-17); **v0.5.1 (9 Sep): perubahan mekanisme log sesi — aturan lokasi di `11_LOG_SESI.md`: log lintas sistem/level repo kini ke folder `_log-sesi/`, bukan root langsung**. v0.4.0: turunan `LOG_SESI` (`_sistem/11_LOG_SESI.md`); v0.4.1: field checkpoint `Pekerjaan belum tersimpan` di `T6_STATUS.md` + sinkronisasi kontrak warisan meta v1.3.0 (tabel Warisan di manifest); **v0.5.0 (9 Sep 2026): folder mandiri — dokumen master yang benar-benar dipakai ikut sebagai salinan berlabel di `_salinan-meta/`, provenance tanpa backtick untuk area master-only** (PR #28, meta v1.12.0).
